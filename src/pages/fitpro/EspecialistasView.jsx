...
import { Stethoscope, Plus, X, Star, Phone, Mail, MapPin, Edit2, Trash2, Heart } from 'lucide-react';
import { useApp, useAuth } from '../../context/FitProContext';

const CARD = '#0d1525';
const BORDER = 'rgba(255,255,255,0.07)';

const emptyEsp = { nome: '', especialidade: 'Nutricionista', email: '', telefone: '', whatsapp: '', descricao: '', valorConsulta: '', disponibilidade: '', parceiro: false, avaliacao: 5.0, percentualComissao: 0, modeloComissao: 'por_contratacao', observacoesComerciais: '', formasPagamento: ['pix'], endereco: { rua: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '', cep: '' } };
...
export default function EspecialistasView() {
  ...
  const filtrados = filtro === 'todos' ? especialistas : filtro === 'parceiros' ? especialistas.filter(e => e.parceiro) : especialistas.filter(e => e.especialidade === filtro);

  const handleSave = () => {
    if (!form.nome.trim()) return alert('Nome é obrigatório');
    const data = { ...form, valorConsulta: parseFloat(form.valorConsulta) || 0, percentualComissao: parseFloat(form.percentualComissao) || 0 };
    if (editId) updateEspecialista(editId, data); else addEspecialista(data);
    setSaved(true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h2 className="text-xl font-bold text-white">Especialistas</h2><p className="text-xs text-slate-500">{especialistas.length} especialista(s)</p></div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtrados.map((esp, i) => (
          <div key={esp.id} className="p-5 rounded-2xl cursor-pointer hover:opacity-90 transition-all"
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto" style={{ background: 'rgba(0,0,0,0.8)' }}>
          <div className="w-full max-w-lg rounded-2xl p-6 my-4" style={{ background: '#0d1525', border: `1px solid ${BORDER}` }}>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={f.field}>
                  ...
                  <input value={form[f.field] || ''} onChange={e => setForm(p => ({ ...p, [f.field]: e.target.value }))} placeholder={f.placeholder} className="w-full px-3 py-2.5 rounded-xl text-sm text-white outline-none" style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.08)' }} />
                </div>
              ))}
              <div>
                <label className="text-xs text-slate-400 block mb-1">Especialidade</label>
                <select value={form.especialidade} onChange={e => setForm(p => ({ ...p, especialidade: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl text-sm text-white outline-none" style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.08)' }}>
                  {ESPECIALIDADES.map(e => <option key={e} value={e}>{e}</option>)}
                </select>
              </div>
              <div><label className="text-xs text-slate-400 block mb-1">Valor da Consulta (R$)</label><input type="number" value={form.valorConsulta} onChange={e => setForm(p => ({ ...p, valorConsulta: e.target.value }))} placeholder="200" className="w-full px-3 py-2.5 rounded-xl text-sm text-white outline-none" style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.08)' }} /></div>
              <div><label className="text-xs text-slate-400 block mb-1">Disponibilidade</label><input value={form.disponibilidade} onChange={e => setForm(p => ({ ...p, disponibilidade: e.target.value }))} placeholder="Seg a Sex: 8h-18h" className="w-full px-3 py-2.5 rounded-xl text-sm text-white outline-none" style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.08)' }} /></div>
              <div><label className="text-xs text-slate-400 block mb-1">Descrição</label><textarea value={form.descricao} onChange={e => setForm(p => ({ ...p, descricao: e.target.value }))} rows={3} className="w-full px-3 py-2 rounded-xl text-sm text-white outline-none resize-none" style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.08)' }} /></div>
            ...
            <button onClick={handleSave} className="w-full mt-4 py-3 rounded-xl font-semibold text-sm text-white" style={{ background: saved ? 'linear-gradient(135deg, #10b981, #059669)' : 'linear-gradient(135deg, #60a5fa, #2563eb)' }}>
